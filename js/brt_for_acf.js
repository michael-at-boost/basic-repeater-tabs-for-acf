...
jQuery(document).ready(function ($) {
  ...
  var TabbedRepeaters = {
    ...
    adminUIFixes: function() {
      ...
      $('.brt-activated > .brt-tabs > .acf-actions [data-event="add-row"]').each(function() {
        var label = $(this).html().replace("Add ", "")
        // width is used to position the remove button inline
        var addBtnWidth = $(this).outerWidth()

        // cache elements
        var container = $(this).closest(".brt-activated")
        var actions = container.find("> .brt-tabs > .acf-actions")
        var handle = container.find("> .acf-repeater > .acf-table > tbody > .acf-row > .acf-row-handle")
        ...
        // update the brt-remove-btn label
        actions.find(".brt-remove-btn").each(function() {
    ...
    attachEventListeners: function() {
      var events = [
        ["sortstop", '.brt-sortable', TabbedRepeaters.handleSort],
      ]
      ...
      acf.addAction('remove', function( $el ){
          ...
          var id = $el.attr("data-brt-id")
          var $tab = $(".brt-tab[data-brt-id='"+id+"']")
          if (!$tab.length) {
            // the tab doesn't exist 
            // e.g. we are in a nested non-tabbed repeater
            return
          }
          var $field = $tab.closest(".brt-activated")
    ...
    addRemoveButton: function($field) {
      ...
      btn.click(function(e) {
        ...
        if ($field.hasClass("brt-vertical")) {
          removeBtn.css({"right" : "calc( 125% - 10px - 0.5em - "+btnCentre+"px )"})
    ...
    findActiveRowRemove: function($field) {
      return $field.find("> .acf-repeater > .acf-table > tbody > .acf-row.active > .acf-row-handle > .-minus")
    },

    // ensures that all tabs are sortable
    refreshSortables: function() {
      $(".brt-nav").each(function() {
        if ($(this).hasClass("ui-sortable")) {
          $(this).sortable("destroy")
        }
        // add a class for the sortstop handler to target
        $(this).addClass("brt-sortable").sortable({cancel: ''})
      })
    },

    // finds the field that an element belongs to
    getField: function($el) {
      return $el.closest(".brt-activated")
    },
    ...
    isTabbedComponent: function($el) {
      ...
      return $repeater.parent().hasClass("brt-activated")
    },
    ...
    getPanels: function($field, $exclude = null) {
      var $panels = $field.find('.acf-table').first().find("> tbody > .acf-row:not(.acf-clone)")
      if ($exclude) {
        // exclusion is used because this set can contain 
        // a panel which is being deleted
        $panels = $panels.filter(function() {
          return $(this).attr("data-brt-id") != $exclude.attr("data-brt-id")
        })
      }
      return $panels
    },
    ...
    activatePanel: function(id) {
      var panel = $('.acf-row:not(.acf-clone)[data-brt-id="'+id+'"]')
      if (panel.length) {
    ...
    activateButton: function(id) {
      var btn = $('.brt-tab[data-brt-id="'+id+'"]')
      if (btn.length) {
    ...
    getNavContainer: function($field) {
      var $container = $field.find("> .brt-tabs")
      if (!$container.length) {
        $container = $('<div class="brt-tabs"></div>')
    ...
    getNav: function($field) {
      ...
      var $nav = $container.find("> .brt-nav")
      if (!$nav.length) {
        var $nav = $('<div class="brt-nav nav"></div>')
    ...
    generateTab: function($panel, idx = null, $field = null) {
      ...
      // tag with an id
      $panel.attr("data-brt-id", id)

      // create a matching tab
      var btn = $("<button class='brt-tab'></button>")
    ...
    handleSort: function( event, ui ) {
      ...
      $target.children().each(function() {
        ids.push( $(this).attr("data-brt-id"))
      })
      ...
      $.each(ids, function(i, id) {
        var $panel = $(".acf-row[data-brt-id='"+id+"']")
